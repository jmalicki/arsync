...
impl HandshakeState {
    ...
    /// # }
    /// ```
    #[allow(clippy::cognitive_complexity)] // State machine complexity is inherent
    #[allow(clippy::too_many_lines)] // State machine requires comprehensive handling
        ...
        T: Transport,
    {
        match self {
            ...
            Self::VersionSent { our_version } => {
                ...
                let mut buf = [0u8; 1];
                read_exact(transport, &mut buf)
                    .await
                    .map_err(|e| anyhow::anyhow!("Failed to read remote protocol version: {e}"))?;
                ...
                if remote_version < MIN_PROTOCOL_VERSION {
                    anyhow::bail!(
                        "Unsupported remote protocol version: {remote_version} (minimum supported: {MIN_PROTOCOL_VERSION})"
                    );
                }
            ...
            Self::VersionNegotiated { protocol_version } => {
                ...
                let mut buf = Vec::new();
                crate::protocol::varint::encode_varint_into(u64::from(our_flags), &mut buf);
            ...
            Self::FlagsSent {
                ...
                // Capability flags are u32 by protocol spec
...
#[must_use]
pub const fn get_our_capabilities() -> u32 {
    let mut flags = 0u32;
